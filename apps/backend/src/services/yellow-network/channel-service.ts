...
export class ChannelService {
  ...
  async createChannel(
    ...
    if (sessionExpiresAt) {
      console.log(
        '[ChannelService] ✅ Session valid until:',
    ...
    if (!this.validateChannelDataShape(channelData)) {
      ...
      console.error(
        '[ChannelService] ❌ Invalid channelData shape:',
    ...
    if (channelIdFromResponse) {
      ...
      if (channelIdFromResponse === ourComputedId) {
        console.log(
          "[ChannelService] ✅ ChannelId matches! Using Yellow Network's provided ID",
        );
      } else {
    ...
    if (!userSignature) {
      ...
      console.log(
        '[ChannelService] ═══ ALLOCATION COMPARISON (Yellow Team Debug) ═══',
      );
      ...
      if (
        ...
        console.warn(
          '[ChannelService] ⚠️  WARNING: Using fallback allocation resolution (Yellow did not provide resolved allocations)',
        );
      ...
      if (dataField !== '0x' && dataField.toLowerCase() !== '0x') {
        console.warn(
          '[ChannelService] ⚠️  WARNING: data field is not "0x"! This may cause hash mismatch!',
        );
      ...
      if (chainId !== 8453 && chainId !== 84532) {
        console.warn(
          `[ChannelService] ⚠️  WARNING: Unexpected chainId! Got ${chainId}`,
        );
      }
      ...
      console.log(
        '[ChannelService] ✅ State encoding: FLATTENED (channelId, intent, version, data, allocations)',
      );
      ...
      try {
        console.log(
          '[ChannelService] ═══ SIGNATURE RECOVERY (Yellow Team Debug) ═══',
        );
        ...
        console.log(
          `[ChannelService] User signature: ${userMatches ? '✅ VALID' : '❌ INVALID'}`,
        );
        console.log(
          `[ChannelService] Server signature: ${serverMatches ? '✅ VALID' : '❌ INVALID'}`,
        );
        ...
        if (!serverMatches) {
          console.warn(
            '[ChannelService] ⚠️  WARNING: Server signature verification FAILED in our off-chain check!',
          );
          console.warn(
    ...
    console.log(
      '[ChannelService] ✅ USING DUAL SIGNATURE APPROACH (0.5.x protocol)',
    );
    console.log(
  ...
  async resyncChannelState(chainId: number): Promise<ChannelWithState | null> {
    ...
    try {
      ...
      console.log(
        `[ChannelService] ✅ Channel state re-synced: ${syncedChannel.channelId}`,
      );
